...
async function getWeather(city) {
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=cac90a419146b7dd70f91a3f6bfa605a&units=metric`
  ).catch(handleError);
  ...
  async function getTime(data) {
    try {
      const response = await fetch(
        `https://secure.geonames.org/timezoneJSON?lat=${data.coord.lat}&lng=${data.coord.lon}&username=nurfatin`
      );
      const responseJson = await response.json();
